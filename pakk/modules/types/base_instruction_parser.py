...
class InstructionParser:
    ...
    def parse_instruction(self, instruction_content: str, instruction: str | None = None, subinstruction: str | None = None):
        if subinstruction is None:
            if self.DEFAULT_SUBINSTRUCTION is None:
                if isinstance(self.INSTRUCTION_NAME, list):
                     return self.parse_instruction(instruction_content, instruction = instruction, subinstruction = instruction)
                raise ValueError(f"No subinstruction given and no default subinstruction set for instruction '{self.INSTRUCTION_NAME}'.")
